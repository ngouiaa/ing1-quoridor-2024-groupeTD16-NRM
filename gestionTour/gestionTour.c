...
#include "historique.h" // Gestion de l'historique des actions
...
void GestionTour(unsigned short int NBJoueurs,
    ...
    while (jeuEnCours) {
        ...
        do {
            ...
            if (choix == 1) {
                ...
                if (TableauBarriereRest[joueurActuel] > 0) {
                    ...
                    scanf("%hu %hu", &x, &y);
                    if (barrierepose(x, y, TableauB)) { // Appelle le programme `poseMur`
                        TableauBarriereRest[joueurActuel]--; // Décrémenter les barrières restantes
                        AjouterHistorique(joueurActuel, x, y, 1); // Enregistrer dans l'historique
                    } else {
                ...
                scanf("%hu %hu", &x, &y);
                if (Deplacement(joueurActuel, x, y, TableauP)) { // Appelle le programme `deplacement`
                    AjouterHistorique(joueurActuel, x, y, 2); // Enregistrer dans l'historique
                } else {
                ...
                AffichageHistorique(); // Appelle le programme `affichageHistorique0`
            } else if (choix == 4) {
                // Annuler un coup
        ...
        if (ConditionVictoire(joueurActuel, TableauP)) { // Fonction à implémenter
